...
const deleteRecordKamag = async (req, res) => {
    ...
    try {
        ...
        await conn.query("UPDATE usterki_kamag SET status = 'usunięte', repair_login =? WHERE id = ?", [userOutlook, id]);
    } catch (error) {
        console.log(error);
    } finally {
        if (conn) conn.end();
...
module.exports = deleteRecordKamag;