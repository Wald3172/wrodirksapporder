...
const sendMailDefectKamag = async (req, res) => {
    const { app_name, kamag, defect, option } = req.body; 
    ...
    const hrefRedirect = '/order/kamag';
    ...
    try {
        ...
        if (typeof(defect) == 'string') {
            await conn.query("INSERT INTO usterki_kamag (kamag, defect, status, notification_login) VALUES (?, ?, 'zgłoszone', ?)", [kamag, defect, userOutlook]);
            defects.push(defect);
        } else {
            defect.forEach(element => {
                conn.query("INSERT INTO usterki_kamag (kamag, defect, status, notification_login) VALUES (?, ?, 'zgłoszone', ?)", [kamag, element, userOutlook]);
        ...
        transporter.sendMail(mailOptions, (error, info) => {
            ...
            res.render('kamag', {title, pageHeader, links, errorInfo, hrefRedirect, footerDepartName});
            } else {
            ...
            res.render('kamag', {title, pageHeader, links, successInfo, hrefRedirect, footerDepartName});
            }
        });
        ...
        const errorInfo = error;
        res.render('kamag', {title, pageHeader, errorInfo, hrefRedirect, footerDepartName, links});
    }
}

module.exports = sendMailDefectKamag;