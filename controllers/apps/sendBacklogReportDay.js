...
const sendBacklogReportDay = async() => {
    let conn;
    try {
        conn = await pool.getConnection();
        let rows = await conn.query("SELECT status, box_no, order_no, box, cot_dsp, cot_date, shift, planner, comment, additional_comment FROM backlog WHERE cot_date = ?", [reportDate]); 
        let lastDate = await conn.query("SELECT myValue FROM info WHERE myKey = 'BacklogReportDay'"); 
        ...
        if (rows.length !== 0 && rDate > lDate) {
            ...
            conn.query("UPDATE info SET myValue = ? WHERE myKey = 'BacklogReportDay'", [reportDate]);
            if (conn) conn.end();
            ...
            let mailOptions = {
                ...
                cc: '',
                subject: `Backlog report ${reportDate}`,
                attachments: [
                    {   
...
module.exports = sendBacklogReportDay;