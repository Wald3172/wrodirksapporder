...
const sendBacklogReportWeek = async() => {
    ...
    try {
        ...
        let lastDate = await conn.query("SELECT myValue FROM info WHERE myKey = 'BacklogReportWeek'"); 
        ...
        if (rows.length !== 0 && rDate > lDate) {
            ...
            conn.query("UPDATE info SET myValue = ? WHERE myKey = 'BacklogReportWeek'", [reportDate]);
            if (conn) conn.end();
...
module.exports = sendBacklogReportWeek;